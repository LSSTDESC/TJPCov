...
def test_get_theta_eff(mock_covariance_real, mock_sacc_file):
    theta_eff, _ = mock_sacc_file.get_theta_xi(
        "galaxy_shear_xi_plus", "src0", "src0"
    )
    assert np.all(theta_eff == mock_covariance_real.get_theta_eff())


def test_get_binning_info(mock_covariance_prj_real):
    # Check we recover the ell effective from the edges
    theta, theta_eff, theta_edges = mock_covariance_prj_real.get_binning_info(
        in_radians=False
    )

    assert np.all(theta_eff == mock_covariance_prj_real.get_theta_eff())
    ...
    (
        ...
        theta_eff2,
        theta_edges2,
    ) = mock_covariance_prj_real.get_binning_info(in_radians=True)
    ...
    with pytest.raises(NotImplementedError):
        mock_covariance_prj_real.get_binning_info("linear")
...
def test_get_cov_WT_spin(mock_covariance_prj_real, tr1, tr2):
    spin = mock_covariance_prj_real.get_cov_WT_spin((tr1, tr2))
...
def test_get_Wigner_transform(mock_covariance_prj_real):
    wt = mock_covariance_prj_real.get_Wigner_transform()

    assert isinstance(wt, WignerTransform)
    assert np.all(wt.ell == np.arange(2, mock_covariance_prj_real.lmax + 1))
    assert np.all(wt.theta == mock_covariance_prj_real.get_binning_info()[0])
    assert wt.s1_s2s == [(2, 2), (2, -2), (0, 2), (2, 0), (0, 0)]


def test_build_matrix_from_blocks(mock_covariance_prj_real):
    s = mock_covariance_prj_real.io.get_sacc_file()
    ndata = s.mean.size
    cov = np.random.rand(ndata, ndata)
    # Make it symmetric
    cov += cov.T

    tracers = mock_covariance_prj_real.get_list_of_tracers_for_cov()
    blocks = []
    ...
    cov2 = mock_covariance_prj_real._build_matrix_from_blocks(blocks, tracers)
...
def test_get_covariance_block(
    mock_covariance_prj_real, tracer_comb1, tracer_comb2
):
    lmax = mock_covariance_prj_real.lmax
    ...
    mock_covariance_prj_real._get_fourier_block = partial(
        override_fourier_block, mock_covariance_prj_real
    )

    WT = mock_covariance_prj_real.get_Wigner_transform()
    s1_s2_1 = mock_covariance_prj_real.get_cov_WT_spin(
        tracer_comb=tracer_comb1
    )
    s1_s2_2 = mock_covariance_prj_real.get_cov_WT_spin(
        tracer_comb=tracer_comb2
    )
    ...
    gcov_xi_1 = mock_covariance_prj_real.get_covariance_block(
    ...
    theta, _, theta_edges = mock_covariance_prj_real.get_binning_info(
        in_radians=False
    )
...
def test__get_covariance_block_for_sacc(
    ...
    nbpw = mock_covariance_prj_real.get_nbpw()
    cov = mock_covariance_prj_real._get_covariance_block_for_sacc(
        tracer_comb1, tracer_comb2
    )
    ...
    dt1 = mock_covariance_prj_real.get_tracer_comb_data_types(tracer_comb1)
    dt2 = mock_covariance_prj_real.get_tracer_comb_data_types(tracer_comb2)
    ...
    for i, dt1i in enumerate(dt1):
        for j, dt2j in enumerate(dt2):
            assert np.all(
                cov[:, i, :, j]
                == mock_covariance_prj_real.get_covariance_block(
                    tracer_comb1,
                    tracer_comb2,
                    pm["minus" in dt1i],
                    pm["minus" in dt2j],
