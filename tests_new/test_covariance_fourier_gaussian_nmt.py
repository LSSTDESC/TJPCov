...
def test_compute_all_blocks():
    ...
    for bi, trs in zip(blocks, tracers_blocks):
        assert np.all(bi == get_covariance_block_for_sacc(cnmt.io.sacc_file,
                                                          trs[0], trs[1]))
...
def test_get_cl_for_cov():
    ...
    cl_cp_code = cnmt.get_cl_for_cov(cl_fid['cl'], cl['nl_cp'], m, m, w,
                                          nl_is_cp=True)
    assert np.abs(cl_cp / cl_cp_code - 1).max() < 1e-10

    # Inputting uncoupled noise.
    nlfill = np.ones_like(cl_fid['ell']) * cl['nl'][0, 0]
    cl_cp_code = cnmt.get_cl_for_cov(cl_fid['cl'], nlfill, m, m, w,
                                          nl_is_cp=False)
    ...
    m2 = np.ones_like(m)
    m2[m != 0] = 0
    assert not np.all(cnmt.get_cl_for_cov(cl, cl['nl_cp'], m, m2, w,
                                               nl_is_cp=True))
...
def test_get_covariance_workspace(kwargs):
    ...
    cl_cov = cnmt.get_cl_for_cov(cl_fid['cl'], cl['nl_cp'], m1, m3, w13,
                                 nl_is_cp=True)
