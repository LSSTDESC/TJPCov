...
class CovarianceFourierGaussianNmt(CovarianceFourier):
    ...
    def get_covariance_block(self, tracer_comb1, tracer_comb2,
        ...
        if np.any(cl[13]) or np.any(cl[24]) or np.any(cl[14]) or \
            ...
            if 'cw' in cache:
                ...
                cw = self.get_covariance_workspace(f[1], f[2], f[3], f[4],
                                                   mn[1], mn[2], mn[3], mn[4],
                                                   lmax=int(ell[-1]),
                                                   **self.nmt_conf['cw'])
    ...
    def get_list_of_tracers_for_cov_without_trs_wsp_cwsp(self):
        """
        Return the covariance independent tracers combinations without those
        used to compute the workspaces and covariance workspaces.
        ...
        for i in [12, 34, 13, 23, 14, 24]:
            ...
            if key in cache:
                ...
                if k in w_by_mask_name_s:
                    ...
                    w_by_mask_name_s[k] = self.get_workspace(fields[i1],
                                                             fields[i2],
                                                             mask_names[i1],
                                                             mask_names[i2],
                                                             bins, **nmt_conf)
